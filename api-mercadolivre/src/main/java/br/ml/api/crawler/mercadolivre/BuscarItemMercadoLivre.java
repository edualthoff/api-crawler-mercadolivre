...
@Getter
public class BuscarItemMercadoLivre implements Serializable, BuscaCrawler<CrawlerItem> {
	...
	private List<CrawlerItem> itens = new ArrayList<>();
	private WebDriver driver;
	
	@Override
	public List<CrawlerItem> analisarUrl(String url) {
	...
	private void varrerHtmlDesktop(WebDriver e) {
		...
		for(int i = 1; i <= e.findElements(By.xpath(".//div/section/ol//li")).size(); i++) {
			...
			CrawlerItem it = new CrawlerItem();
			...
			it.setCrawlerItemPK(new CrawlerItem.CrawlerItemPK());
			if (m.find()) {
				it.getCrawlerItemPK().setId(m.group().replace(".com.br/", "").trim());
			}
			it.getCrawlerItemPK().setOrigem(CrawlerBuscaSiteBase.MERCADOLIVRE.getSiteBase());
			//System.out.println("site: "+it.getCrawlerItemPK().getOrigem()+" id: "+it.getCrawlerItemPK().getId());
			itens.add(it);
